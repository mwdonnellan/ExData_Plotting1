...
#                       
#                       Plot 4 is a 4-panel display of 4 charts of data from the
#						source data file:
#
#						1) Global Active Power by time, recorded once per minute
#						2) Voltage, recorded once per minute
#						3) Energy sub metering, 3 data series
#						4) Global reactive power, recorded once per minute
#
#       Author: mwd
#
...
#2 set the 2 X 2 layout with par(mfrow=c(2,2))
#3 draw the Global_active_power by datetime chart
#4 draw the Voltage by datetime chart
#5 draw the energy sub metering chart with its 3 series
#6 draw the Global_reactive_power chart
#7 close the png device

png(filename = "plot4.png",width = 480, height = 480, units = "px", pointsize = 12, bg = "white", res = NA, family = "", restoreConsole = TRUE, type = c("windows", "cairo", "cairo-png"))
...
#Voltage chart
plot(dmy_hms(euSubSet$DateTime), euSubSet$V5, type="l", xlab = "datetime", ylab = "Voltage")
...
legend('topright', lCaptions, col=lColors, lty=1, lwd=1, cex=0.9)

#Global reactive power chart
plot(dmy_hms(euSubSet$DateTime), euSubSet$V4, type="l", xlab = "datetime", ylab = "Global_reactive_power")
